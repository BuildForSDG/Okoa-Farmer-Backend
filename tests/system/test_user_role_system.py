...
class TestUserSystem(TestBase):
    ...
    def test_get_user_not_found(self):
        with app.test_client() as client:
            with self.app_context():
                resp = client.get('/api/user/roles/1/1', headers={'Authorization': self.access_token})
    ...
    def test_register_duplicate_user(self):
        with app.test_client() as client:
            with self.app_context():
                ...
                client.post('/api/user/roles', data=user_role_dict, headers={'Authorization': self.access_token})
                response = client.post('/api/user/roles', data=user_role_dict, headers={'Authorization': self.access_token})
    ...
    def test_get_single_user(self):
        with app.test_client() as client:
            with self.app_context():
                client.post('/api/user/roles/1/1', data=user_dict, headers={'Authorization': self.access_token})
                resp = client.get('/api/user/roles/1/1', headers={'Authorization': self.access_token})
                self.assertEqual(resp.status_code, 200)
    ...
    def test_delete_user(self):
        with app.test_client() as client:
            with self.app_context():
                client.post('/api/user/roles/1/1', data=user_dict, headers={'Authorization': self.access_token})
                resp = client.delete('/api/user/roles/1/1', headers={'Authorization': self.access_token})
                self.assertEqual(resp.status_code, 200)
